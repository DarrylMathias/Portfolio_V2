    ...
    >
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 p-6">
        {skills.map((item, idx) => (
          <li
            key={idx}
            className="w-full relative rounded-2xl border border-b-0 border-slate-800 p-5 md:p-8"
            style={{
          ...
          >
            <a href={item.link} target = '_blank'>
              <blockquote>
                ...
                <span className="relative z-20 text-sm md:text-base leading-[1.6] text-white font-normal py-5">
                  {item.description}
                </span>
              </blockquote>
            </a>
